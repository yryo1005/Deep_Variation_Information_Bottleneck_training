...
4. **問い → 予想 → 実験 → 結果 → 考察 → 発展** — 各解説 Markdown は，実験前の予想と実験後の考察を分け，結果・解説・発展課題の回答例は折りたたみにする．実装できることより，自分で仮説を立てて検証することを先に置く．
...
各実験は次の順で進める．折りたたみ（「実験結果を見る」「解説を見る」「回答例を見る」）は，自分の答えを書いてから開く．

1. 対応する `.md` の **実験前に考える** に答える（001 は学習と検証の Accuracy，004 は「何を残して何を捨てるか」）．
2. 実験001から順に `.ipynb` を Colab で開き，ランタイムを **GPU** に変更する（またはローカル Jupyter で実行）．
3. 学習後に **実験結果** を開き，予想と照合する．続いて **実験後に考える** に答えてから **解説** を開く．
4. **発展課題** で定数や損失を少し変え，本実験の結果と比較する．回答例は最後に開く．
5. `figures/` の画像は `.md` 内で参照される．
