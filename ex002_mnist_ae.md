...
1. クラスラベルを使わず，画像をいったん 32 次元に圧縮してから戻すと，数字の形は残ると思いますか．残る／残らないと，その理由を書いてください．
2. 再構成画像は入力より鮮明になると思いますか，ぼやけると思いますか．
3. 実験001の分類器は $Y$ の情報を直接最大化しました．本実験の $z$ は，入力 $X$ の情報を残す方向と捨てる方向のどちらに働くと思いますか．
...
1. 実際の再構成は，実験前の予想と一致しましたか．
2. 検証 MSE は単調に下がっています．実験001で見えた「学習は良くなるが検証は悪化する」現象が，ここでは目立たないのはなぜでしょうか．
3. 再構成が平滑になることから，$z$ に残っている情報と捨てられた情報を区別できますか．
4. ラベルなしで字形が残るなら，分類に必要な情報は画素全体のうちどの部分にあると考えられますか．
...
潜在次元を小さくしたとき，再構成の劣化と数字の判読しやすさがどう変わるかを調べてください．

1. **何を変更するか**: 実験設定の `LATENT_DIM = 32` を `2` または `8` にする．Encoder 最終層と Decoder 初層は `latent_dim` を参照しているので，この定数だけ変えればよいです．
2. **何を比較するか**: 検証 MSE / MAE と，再構成画像で数字が読めるかを，32 次元の結果と比べる．
3. **予想**: 次元を落とすほど MSE は増え，再構成はぼやけます．ただし MNIST は単純なので，8 次元でも字形の大枠は残ると予想されます．
...
既存結果を上書きしないよう，学習前に `outputs/ex002_mnist_ae/Adam/0.001_128/0/` を退避するか，`EXPERIMENT_NAME` を一時的に変えてください．その後ノートブックの学習セルを再実行します．

### 実験方法

同じ検証バッチの再構成画像を 32 次元と 8 次元（または 2 次元）で並べ，MSE と目視の判読を記録します．

### 期待される結果

8 次元でも数字の種類はおおよそ分かり，MSE は 32 次元より大きくなります．2 次元では輪郭が崩れ，4 と 9 など似た数字が混ざりやすくなります．

### 結果から分かること

ボトルネック次元は「何を残すか」の容量です．後続の VAE / Deep VIB では，この容量に確率的な圧縮（KL）を加えて，残す情報の種類まで制御します．
