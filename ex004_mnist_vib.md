...
次の問いに自分の仮説を書いてから，2.2 節へ進んでください．

1. MNIST の「7」を分類するために，画像の全画素情報は本当に必要でしょうか．
2. $x \to z \to y$ としたとき，$z$ に分類に不要な情報まで残っていたら，何が起きるでしょうか．
3. 必要な情報は残したい，不要な情報は捨てたい，というとき，「残っている情報量」をどう評価すればよいでしょうか．

### 2.2 なぜ情報を圧縮するのか

実験001は画素から直接 $Y$ を予測した．実験002・003は $X$ を $z$ に通して再構成した．分類だけが目的なら，$z$ に残すべきなのは **$Y$ の予測に効く情報** である．$x \to z \to y$ としたとき，$z$ に筆圧の揺れや背景のゴミまで残すと，学習データには合っても検証では汎化しにくい．

必要な情報は残したい．不要な情報は捨てたい．残っている量を相互情報量で書くと，$I(Z; Y)$ は大きく，$I(Z; X)$ は小さくしたい．これが Information Bottleneck である．実装では直接 $I$ を計算できないので，変分近似により次の和にする．交差エントロピーが $I(Z; Y)$ を押し上げ，KL が $I(Z; X)$ の上界を抑える．
...
1. 実験001（交差エントロピーだけ）と比べて，ボトルネックを挟むと検証 Accuracy は下がると思いますか．
2. 実験003の VAE では KL が 0 に潰れませんでした．分類損失に差し替えた本実験では，KL は 0 に潰れると思いますか．
3. 学習 Accuracy が $99\%$ 付近まで上がったとき，検証 Accuracy も同じだけ上がると思いますか．
...
1. 検証 Accuracy は実験001（$0.9808$）と比べてどうでしたか．ボトルネックを挟んでも精度が大きく落ちなかったことから，何が分かりますか．
2. KL は 0 に潰れましたか．交差エントロピーが $I(Z; Y)$ を押し上げることと，どう関係しますか．
3. 学習 Accuracy は $0.9948$，検証は $0.9780$ 付近です．$\beta$ を大きくすると，この差はどうなると思いますか．
4. 「不要な情報を捨てたい」は，実装ではどの項が担っていますか．
...
圧縮の強さ $\beta$ を変えて，識別精度と KL（圧縮の指標）がどう動くかを調べてください．これが Information Bottleneck のトレードオフそのものです．

1. **何を変更するか**: `BETA = 0.001` を `0.01` と `0.0001` にする．
2. **何を比較するか**: 最良検証 Accuracy と，最終付近の検証 KL．
3. **予想**: $\beta$ を大きくすると KL は小さく（圧縮が強い），Accuracy はわずかに下がる可能性があります．$\beta$ を小さくすると KL は大きく，実験001の全結合分類器に近づきます．
...
`loss_func` 自体は変えなくてよく，`BETA` だけ変えれば十分です．条件ごとに出力ディレクトリが衝突しないよう，学習前に既存の `outputs/ex004_mnist_vib/Adam/0.001_128/0/` を退避してください．

### 実験方法

各 $\beta$ で seed 0，10 エポック学習し，`val_acc` の最大と最終 `val_kl` を表にします．

### 期待される結果

$\beta=0.001$ では検証 Accuracy 約 $0.978$，KL 約 $22$ です．$\beta$ を 10 倍にすると KL は下がりやすく，Accuracy は同水準かやや低下します．

### 結果から分かること

目的関数 $\mathcal{L}_{\mathrm{CE}} + \beta \mathcal{L}_{\mathrm{KL}}$ は，「$I(Z;Y)$ を大きくしたい」「$I(Z;X)$ を小さくしたい」を実装可能な和にしたものです．$\beta$ はその比重です．
