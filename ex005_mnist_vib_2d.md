...
1. 良い特徴表現とは何でしょうか．同じクラスの点は近く，異なるクラスの点は遠くなるべきでしょうか．
2. Accuracy が高いモデルと，潜在空間が「きれいに」分かれて見えるモデルは，同じものでしょうか．
3. 潜在次元を 32 から 2 に落とすと，Accuracy はどのくらい下がると思いますか．3 と 5 のような似た数字は，平面上でどう配置されると思いますか．
4. 決定領域は，散布した点の密度から決まると思いますか．それとも分類器が平面をどう切るかから決まると思いますか．
...
1. 散布図は予想どおり「クラスごとに塊」でしたか，それとも放射状でしたか．その形は，KL と分類項のどちらが作っていると考えられますか．
2. 検証 Accuracy は 32 次元より約 $1.8$ ポイント低いです．それでも $96\%$ を超えることから，分類に必要な情報の量について何が言えますか．
3. Accuracy が高いことと，3 と 5 が原点で重ならないことは同じでしょうか．
...
$\beta$ を変えた 2 次元潜在空間を描き，「きれいな分離」と Accuracy が同時に動くかを調べてください．

1. **何を変更するか**: `BETA = 0.001` を `0.01` にする（必要なら `0.0001` も）．
2. **何を比較するか**: 検証 Accuracy，KL，および $\mu$ の散布図（放射状か，原点に潰れるか）．
3. **予想**: $\beta$ を大きくすると点が原点へ寄り，クラス間の隙間が狭くなる一方，圧縮は強くなります．Accuracy は下がるかもしれません．
...
学習後，既存の散布図セル（検証 $\mu$ を `classify` の格子と重ねるセル）をそのまま実行します．`n_grid = 200` は変えなくてよいです．

### 実験方法

本実験の図と，$\beta=0.01$ の図を並べ，原点付近の重なりと外周の扇の幅を目視で比べます．

### 期待される結果

$\beta$ を大きくするとクラスタは原点に近づき，3 と 5 の重なりが見やすくなります．Accuracy は本実験の $0.9608$ より下がることが多いです．

### 結果から分かること

「きれいな潜在空間」は見た目の好みではなく，IB のトレードオフの現れです．Accuracy だけ見ていても，表現がどう圧縮されているかは分かりません．
