...
$$
\mu = W_{\mu} h_{2} + b_{\mu} \in \mathbb{R}^{2},\quad
\log\sigma^{2} = W_{\log\sigma^{2}} h_{2} + b_{\log\sigma^{2}} \in \mathbb{R}^{2}
$$
...
可視化ではサンプリングを行わず，検証画像ごとの $\mu$ を座標 $(\mu_{1}, \mu_{2})$ として用いる．

### 2.2 決定領域の計算

決定領域（分離領域）は，散布した点の密度から境界を推定するのではなく，潜在平面を格子に切って分類器へ通すことで求める．手順は次のとおりである．
...
3. 各格子点を線形分類器 `classify` に通し，予測クラス $\hat{y}_{i,j}=\operatorname{argmax}_{k}(W z_{i,j}+b)_{k}$ を得る
...
1. 良い特徴表現とは何か．同じクラスの点は近く，異なるクラスの点は遠くなるべきか．
2. Accuracy が高いモデルと，潜在空間が「きれいに」分かれて見えるモデルは，同じものか．
3. 潜在次元を 32 から 2 に落とすと，Accuracy はどのくらい下がると考えるか．3 と 5 のような似た数字は，平面上でどう配置されると考えるか．
4. 決定領域は，散布した点の密度から決まると考えるか．それとも分類器が平面をどう切るかから決まると考えるか．
...
1. 散布図は予想どおり「クラスごとに塊」であったか，それとも放射状であったか．その形は，KL と分類項のどちらが作っていると考えられるか．
2. 検証 Accuracy は 32 次元より約 $1.8$ ポイント低い．それでも $96\%$ を超えることから，分類に必要な情報の量について何が言えるか．
3. Accuracy が高いことと，3 と 5 が原点で重ならないことは同じか．
...
$\beta$ を変えた 2 次元潜在空間を描き，「きれいな分離」と Accuracy が同時に動くかを調べよ．

1. **何を変更するか**: `BETA = 0.001` を `0.01` にする（必要なら `0.0001` も）．
2. **何を比較するか**: 検証 Accuracy，KL，および $\mu$ の散布図（放射状か，原点に潰れるか）．
3. **予想**: $\beta$ を大きくすると点が原点へ寄り，クラス間の隙間が狭くなる一方，圧縮は強くなる．Accuracy は下がるかもしれない．
...
学習後，既存の散布図セル（検証 $\mu$ を `classify` の格子と重ねるセル）をそのまま実行する．`n_grid = 200` は変えなくてよい．

### 実験方法

本実験の図と，$\beta=0.01$ の図を並べ，原点付近の重なりと外周の扇の幅を目視で比べる．

### 期待される結果

$\beta$ を大きくするとクラスタは原点に近づき，3 と 5 の重なりが見やすくなる．Accuracy は本実験の $0.9608$ より下がることが多い．

### 結果から分かること

「きれいな潜在空間」は見た目の好みではなく，IB のトレードオフの現れである．Accuracy だけ見ていても，表現がどう圧縮されているかは分からない．
