...
本実験の目的は，実験004と同じ Deep Variational Information Bottleneck（Deep VIB）で MNIST を分類しつつ，似た数字同士を取り違えたときの損失を大きくすることである．交差エントロピー（CCE）そのものは残し，クラス間の重みで CCE をスケールする．
...
= -\frac{1}{N}\sum_{n=1}^{N}
\left(1 + \sum_{k \neq y_{n}} W_{y_{n} k}\, p_{n,k}\right)
\log p_{n,y_{n}}
$$

である．括弧内は通常の CCE に対する倍率である．正解 $y$ のサンプルが，似たクラス $k$ へ確率を漏らすほど倍率が 1 より大きくなり，$-\log p_{y}$ が強調される．
...
1. 3 を 4 と間違えることと，3 を 8 と間違えることは，同じ間違いか．
2. クラス間の類似性を損失関数に組み込むと，Accuracy や潜在空間の配置は何が変わると考えるか．
3. その「似ている」という関係は，誰が，どのように決めるべきか（人手の字形，混同行列，学習可能な重み，など）．
4. 重み付き CCE は通常の CCE と勾配の向きが近いので，2 次元散布は同じ形のままスケールだけ変わると考えるか．それとも配置自体が変わると考えるか．
...
左が CCE，右が重み付き CCE である．どちらも原点から放射状にクラスが並び，方位の割り当てはほぼ同じである．重み付き CCE は $-\log p_{y} $ に倍率を掛けるだけなので，勾配の向きは通常の CCE に近く，扇状の配置は保たれる．一方 $\mu$ の範囲は CCE がおおよそ $[-15,21]$，重み付き CCE が $[-22,24]$ と広く，数字 5 のクラスタはやや伸びる．決定領域は実験005と同じく，潜在平面の $200\times 200$ 格子を `classify` に通して塗っている．$\mu$ からの検証 Accuracy は CCE $0.9604$，重み付き CCE $0.9538$ である．

</details>

### 実験後に考える

1. 32 次元の Accuracy は通常の CCE とほぼ同じであった．「損失を変えたのに精度が同じなら意味がない」と言えるか．
2. 2 次元では配置の方位はほぼ同じで，スケールと KL が変わった．実験前の予想と一致したか．なぜ Brier ほど配置が変わらないのか．
3. $W_{3,5}$ を人手で $4.5$ にしたことは妥当か．データから決めるには，何を観測すればよいか．
...
通常の CCE は $-\log p_{y}$ のみを見る．本実験では，似たクラスへ漏れた確率 $p_k$ が倍率を押し上げる．32 次元では Accuracy は実験004と同水準かそれ以上であり，重み付けが学習を壊していない．
...
「似ている」対の重みを意図的に入れ替え，2 次元配置や混同の仕方が変わるかを調べよ．
...
3. **予想**: 3 と 5 の罰を弱めると，2 次元では両者の重なりが残るか増える．無関係な対を強く罰しても，字形が違うため配置はあまり動かないかもしれない．
...
長さ 50 のベクトル規約（$i<j$ の 45 個 + 対角 5 個）は変えないこと．比較用の 2 次元学習は，ノートブック後半の `COMPARISON_SETTINGS` で `("wce", 2)` を再実行する．既存の `compare/wce_2/` を退避してから学習せよ．

### 実験方法

元の重みと改変した重みで，潜在 2 次元の散布図と検証 Accuracy を並べよ．

### 期待される結果

$(3,5)$ を弱めると 3 と 5 の境界が甘くなり，Accuracy がわずかに変わることがある．$(0,1)$ を強くしても，0 と 1 はもともと離れやすいので，配置の変化は限定的である．

### 結果から分かること

損失の設計は「何を間違えてほしくないか」の宣言である．類似度を人手で置くことは可能であるが，その妥当性はデータと目的に依存する．混同行列から $W$ を作る，あるいは $W$ を学習する，が次の研究的な問いである．
